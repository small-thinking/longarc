...
`profit_pace_mode: "only"` replaces a fixed profit threshold with one rule: gross premium capture at the ask must strictly exceed both `profit_pace_min_capture_fraction` and the fraction of the current leg's lifetime already elapsed. Pass the actual `facts.opening_executed_at` timestamp; the comparison lifetime ends at 16:00 New York time on `expiry_date` as a planning convention, not an exercise cutoff. Missing opening time leaves a potentially reachable pace exit unknown. Older policies without this mode retain their fixed threshold; a stored policy with a pace minimum but no mode retains its original additive behavior. Positive fee-adjusted P&L and risk-exit priority remain required. Replay supplies each simulated leg's opening timestamp to the same evaluator.
