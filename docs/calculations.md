...
An optional `profit_pace_min_capture_fraction` adds an earlier profit exit when gross premium capture at the ask strictly exceeds both this minimum and the fraction of the current leg's lifetime already elapsed. Pass the actual `facts.opening_executed_at` timestamp; the comparison lifetime ends at 16:00 New York time on `expiry_date` as a planning convention, not an exercise cutoff. Missing opening time leaves a potentially reachable pace exit unknown. The existing fixed profit threshold, positive fee-adjusted P&L requirement and risk-exit priority remain in force. Replay supplies each simulated leg's opening timestamp to the same evaluator.
