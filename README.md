...
QQQ decisions use the current ignored policy at `private/qqq-covered-call-plan/decision-policy.json`; older dated review files retain their historical rules. The decision engine supports strict fixed profit thresholds and an optional earlier profit-pace trigger based on actual leg opening time, premium captured at the ask, and elapsed lifetime. Risk exits retain priority and profit exits still require positive fee-adjusted P&L. Missing current policy, opening time or quote evidence remains a visible analysis gap.
