...
QQQ decisions use the current ignored policy at `private/qqq-covered-call-plan/decision-policy.json`; older dated review files retain their historical rules. The current QQQ policy uses a single profit-pace threshold based on actual leg opening time, premium captured at the ask, and elapsed lifetime. The engine still reads older fixed-threshold policies for historical analysis. Risk exits retain priority and profit exits still require positive fee-adjusted P&L. Missing current policy, opening time or quote evidence remains a visible analysis gap.
