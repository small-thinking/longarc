...
QQQ decisions use the current ignored policy at `private/qqq-covered-call-plan/decision-policy.json`; older dated review files retain their historical rules. The decision engine supports an optional strict profit threshold operator so a policy can require gross premium capture greater than its threshold. Missing current policy or quote evidence remains a visible analysis gap.
